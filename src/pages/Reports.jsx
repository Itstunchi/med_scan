...
import { FileText, Clock, CheckCircle2, AlertCircle, Search, ChevronRight, Upload, ArrowLeft, ArrowUpDown, LayoutGrid, List, Share2 } from 'lucide-react'
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-5 sm:space-y-6 px-4 sm:px-0 animate-fade-in">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <div className="flex gap-2">
          <div className="flex flex-1 gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1 scrollbar-thin sm:flex-none">{filters.map((f) => <button key={f.value} onClick={() => setFilter(f.value)} className={`shrink-0 rounded-lg px-3 py-1.5 text-sm font-medium transition-all ${filter === f.value ? 'bg-teal-700 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>{f.label}</button>)}</div>
          <button
          ...
          <div className="hidden shrink-0 gap-1 rounded-xl border border-slate-200 bg-white p-1 sm:flex">
            <button onClick={() => setView('list')} className={`rounded-lg p-1.5 ${view === 'list' ? 'bg-teal-700 text-white' : 'text-slate-500 hover:bg-slate-100'}`} title="List view">
              <List className="h-4 w-4" />
            </button>
            <button onClick={() => setView('grid')} className={`rounded-lg p-1.5 ${view === 'grid' ? 'bg-teal-700 text-white' : 'text-slate-500 hover:bg-slate-100'}`} title="Grid view">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((report) => (
            <Link key={report.id} to={`/reports/${report.id}`} className="card group flex flex-col gap-3 p-4 transition-colors hover:bg-slate-50">
              <div className="flex items-start justify-between">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-50"><FileText className="h-5 w-5 text-teal-700" /></div>
