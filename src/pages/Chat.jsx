...
import { MessageSquare, Send, Loader2, Bot, User as UserIcon, Sparkles, ArrowLeft, Plus, Copy, Check, RotateCcw, ThumbsUp, ThumbsDown, History, X as CloseIcon } from 'lucide-react'
  ...
  return (
    <div className="mx-auto flex h-[calc(100dvh-6rem)] md:h-[calc(100dvh-8rem)] w-full max-w-4xl flex-col animate-fade-in px-3 sm:px-4 md:px-0">
      ...
      <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:mb-3">
        <Link to="/dashboard" className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-medium text-slate-500 hover:text-teal-700 transition-colors">
      ...
      {showHistory && (
        <div className="mb-2 sm:mb-3 max-h-48 sm:max-h-56 overflow-y-auto rounded-xl sm:rounded-2xl border border-slate-200 bg-white p-2.5 sm:p-3 shadow-lg animate-fade-in">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-[10px] sm:text-xs font-semibold uppercase tracking-wide text-slate-400">Past Questions</p>
            <button onClick={() => setShowHistory(false)} className="p-1 hover:bg-slate-100 rounded transition-colors">
              <CloseIcon className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-slate-400 hover:text-slate-600" />
            </button>
          </div>
          {historyLoading ? (
            <div className="space-y-1.5 sm:space-y-2">
              {[1, 2, 3].map((i) => <div key={i} className="h-7 sm:h-8 animate-pulse rounded-lg bg-slate-100" />)}
            </div>
          ) : history.length === 0 ? (
            <p className="py-2 text-center text-xs sm:text-sm text-slate-400">No past questions yet.</p>
          ) : (
            <div className="space-y-0.5 sm:space-y-1">
              {history.map((h, i) => (
                <button
                  key={i}
                  onClick={() => { setInput(h.content); setShowHistory(false); }}
                  className="block w-full truncate rounded-lg px-2 sm:px-2.5 py-1 sm:py-1.5 text-left text-xs sm:text-sm text-slate-600 hover:bg-slate-50 transition-colors"
                >
                  {h.content}
                </button>
              ))}
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-3 sm:px-4 md:px-5 py-2.5 sm:py-3">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-sm sm:text-base font-semibold text-slate-800 truncate">AI Health Assistant</h2>
              <p className="text-[10px] sm:text-xs text-slate-400 truncate">Ask about your reports or health questions</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-3 sm:space-y-4 overflow-y-auto p-3 sm:p-4 md:p-5 scrollbar-thin">
          {messages.length === 0 && (
            <div className="flex h-full flex-col items-center justify-center gap-3 sm:gap-4 text-center px-2">
              ...
              <div>
                <h3 className="text-sm sm:text-base font-semibold text-slate-700">How can I help you today?</h3>
                <p className="mt-1 text-xs sm:text-sm text-slate-500">Ask me about your medical reports, lab results, or general health questions.</p>
              </div>
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={i} className={`flex gap-2 sm:gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`flex h-6 w-6 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full ${msg.role === 'user' ? 'bg-teal-100' : 'bg-teal-50'}`}>
                  {msg.role === 'user' ? (
                ...
                <div className={`group flex max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`rounded-lg sm:rounded-2xl px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm leading-relaxed break-words ${msg.role === 'user' ? 'bg-teal-700 text-white' : 'bg-slate-100 text-slate-700'}`}>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-1.5 sm:gap-2 px-1">
                    {msg.time && <span className="text-[9px] sm:text-[10px] text-slate-400">{formatTime(msg.time)}</span>}
                    {msg.role === 'assistant' && (
                      <div className="flex items-center gap-1.5 sm:gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                        <button 
                        ...
                        <button 
                          ...
                          className="p-1 hover:bg-slate-200 rounded transition-colors"
                        >
                          <ThumbsUp className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${feedback[i] === 'up' ? 'fill-teal-600 text-teal-600' : 'text-slate-400 hover:text-slate-600'}`} />
                        </button>
                        <button 
                          ...
                          className="p-1 hover:bg-slate-200 rounded transition-colors"
                        >
                          <ThumbsDown className={`h-3 w-3 sm:h-3.5 sm:w-3.5 ${feedback[i] === 'down' ? 'fill-rose-500 text-rose-500' : 'text-slate-400 hover:text-slate-600'}`} />
                        </button>
                      </div>
                    )}
                  </div>
          ...
          {loading && (
            <div className="flex gap-2 sm:gap-3">
              ...
              <div className="flex items-center gap-1 rounded-lg sm:rounded-2xl bg-slate-100 px-3 sm:px-4 py-2 sm:py-3">
                <div className="h-1.5 w-1.5 sm:h-2 sm:w-2 animate-bounce rounded-full bg-slate-400 [animation-delay:-0.3s]"></div>
                <div className="h-1.5 w-1.5 sm:h-2 sm:w-2 animate-bounce rounded-full bg-slate-400 [animation-delay:-0.15s]"></div>
                <div className="h-1.5 w-1.5 sm:h-2 sm:w-2 animate-bounce rounded-full bg-slate-400"></div>
              </div>
            </div>
          )}
        ...
        <div className="border-t border-slate-100 p-2.5 sm:p-3 md:p-4">
          ...
          <div className="mt-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1.5 sm:gap-2">
            ...
            {input.length > MAX_CHARS * 0.8 && (
              <span className={`shrink-0 text-[10px] sm:text-xs whitespace-nowrap ${input.length >= MAX_CHARS ? 'text-rose-500' : 'text-slate-400'}`}>
